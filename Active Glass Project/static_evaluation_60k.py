...
OUT_IMAGE = 'static_frame_60k.png'

def main():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Device: {device}")
    
    print("Loading graph for T=60000...")
    ...
    print("Running full-batch inference on T=60000...")
    with torch.no_grad():
        pred_log = model(data).cpu().numpy().flatten()
        
    # Inverse Transformation
    print("Applying np.exp() inverse transformation...")
    gt_real = np.exp(gt_log)
    pred_real = np.exp(pred_log)
    
    # Find shared color scale
    vmin = np.percentile(gt_real, 5)   # Trim outliers for better contrast
    ...
    ax1.set_title(f"Ground Truth Propensity (T=60000)")
    ...
    ax2.set_title(f"EGNN Prediction (T=60000)")
